...
BEGIN
   ...
   IF (nvl(Project_Cost_Element_API.Get_Obsolete(newrec_.company, newrec_.project_cost_element),'FALSE') = 'TRUE') THEN
      Error_SYS.Record_General(lu_name_, 'BLOCKED: Project Cost/Revenue Element :P1 is Blocked for Use.', newrec_.project_cost_element);
   END IF;
...
FUNCTION Get_Project_Follow_Up_Element(
   ...
   posting_type_                  IN VARCHAR2 DEFAULT NULL ) RETURN VARCHAR2
IS
   temp_                          COST_ELEMENT_TO_ACCOUNT_TAB.project_cost_element%TYPE;
...
EXCEPTION
   ...
   WHEN cost_element_null THEN
      IF generate_errors_ ='TRUE' THEN
         Error_SYS.Record_General(lu_name_, 'COSTELMISNULL: There is no project cost/revenue element connected to code part value :P1',code_part_value_);
      ELSE
         RETURN NULL;
      END IF;
...
BEGIN
   RETURN Get_Project_Follow_Up_Element(company_, code_rec_.code_a, code_rec_.code_b, code_rec_.code_c,
                                    ...
                                    generate_errors_, posting_type_);
END Get_Project_Follow_Up_Element;
