...
--  151211  Chwtlk  Bug-126216, Modified Is_Voucher_Type_Used_Internal()
--  160325  Savmlk  Bug 128254, Modified the Check_Update___().
-----------------------------------------------------------------------------
...
micro_cache_id1_ voucher_type_tab.company%TYPE;

micro_cache_id2_ voucher_type_tab.voucher_type%TYPE;
...
BEGIN
   super(attr_);
   Client_SYS.Add_To_Attr( 'LABLE_PRINT', Lable_Print_API.decode('N'), attr_ );
...
BEGIN   
   ...
   IF NOT (Check_Simulation_Vou_Ok__(newrec_.voucher_type, newrec_.simulation_voucher, newrec_.company)) THEN
      Error_SYS.Record_General(lu_name_,'SIMUVOUNOTOK: Simulation Voucher can be checked for Voucher Types that are connected only to the Function Groups "M", "K", "Q" or "R" ');
   END IF;
...
BEGIN
   ...
   IF (indrec_.automatic_allot) THEN      
      ...
      --Bug 128254, Begin Changed the condition and error message.
   ...
   IF ((newrec_.ledger_id IS NOT NULL) AND (newrec_.ledger_id NOT IN ('*', '00' ))) THEN
      FOR rec_ IN get_function_group LOOP
         IF (get_function_group%NOTFOUND ) THEN
            EXIT;
         ELSE
            IF (rec_.function_group NOT IN ('A','K','M','Q','R','X','Z','P', 'PPC')) THEN
               Error_SYS.Record_General(lu_name_,'INTNOALLOW: Only Voucher Types that are connected to Function Groups "A", "K", "M", "Q", "R", "X", "Z", "P", "PC" are allowed in Internal Ledger');
            END IF;
            IF (rec_.function_group = 'PPC') THEN
               ...
               IF (nvl(il_project_accounting_, 'FALSE') = 'FALSE') THEN
                  Error_SYS.Record_General(lu_name_,'PCPROJACCOUNTING: Not allow to connect a PPC function group voucher type to a non-project IL.');
               END IF;
            END IF;
         END IF;
      END LOOP;
   ...
   --Validate voucher types defined using function group PC to have balance set TRUE
    IF (newrec_.balance ='FALSE' AND Get_Voucher_Group(newrec_.company, newrec_.voucher_type) = 'PPC') THEN
       Error_SYS.Record_General(lu_name_,'VOUGRPPCBAL: Balance Mandatory check box should be seleted if voucher type is connected to function group "PC" ');
    END IF;
...
IS
BEGIN
   IF( voucher_group_   IN ('A','B','G','L','P','R','T','U','W','X','Z','PPC' ) AND
       automatic_allot_ != 'Y' )
   THEN
...
BEGIN
   ...
   IF (simulation_voucher_  = 'TRUE') THEN
      FOR rec_ IN get_vou_functions LOOP
         IF (get_vou_functions%NOTFOUND) THEN
            flag_ := FALSE;
            EXIT;
         ELSIF (rec_.function_group NOT IN ('M','K','Q','R')) THEN
            flag_ := FALSE;
            EXIT;
         END IF;
      END LOOP;
...
BEGIN
   ...
   ELSE
      ...
      IF NOT (function_group_ IN ('B','G','M','N','U')) THEN
         IF (single_function_ <> 'Y') THEN
...
BEGIN
   $IF Component_Intled_SYS.INSTALLED $THEN
      Internal_Ledger_API.Exist ( company_,
                                  ledger_id_ );
   $ELSE
      Error_SYS.Record_General(lu_name_, 'NOTALLOWED: Ledger Id :P1 is not allowed since Internal Ledger is not installed',ledger_id_);
   $END
...
BEGIN
   ...
   IF (check_automatic%NOTFOUND) THEN
...
-- Bug 126216, Begin, Included accounting_year_ as a parameter.
...
BEGIN
   is_used_ := 'FALSE';
   $IF Component_Intled_SYS.INSTALLED $THEN
      -- Bug 126216, Begin, Included accounting_year_ as a parameter in the function call.
...
IS
   objid_               VOUCHER_TYPE.objid%TYPE;
   objversion_          VARCHAR2(2000);
   info_                VARCHAR2(1000);
   attr_                VARCHAR2(2000);
...
BEGIN
   ...
   Client_SYS.Add_To_Attr('LABLE_PRINT_DB', lable_print_, attr_);
   Client_SYS.Add_To_Attr('AUTOMATIC_ALLOT_DB', automatic_, attr_);
   Client_SYS.Add_To_Attr('AUTOMATIC_VOU_BALANCE_DB', optional_auto_bal_, attr_);
   Client_SYS.Add_To_Attr('STORE_ORIGINAL_DB', store_original_, attr_);
   Client_SYS.Add_To_Attr('SINGLE_FUNCTION_GROUP', single_, attr_);
   Client_SYS.Add_To_Attr('LEDGER_ID', ledger_id_, attr_);
   Client_SYS.Add_To_Attr('USE_MANUAL', manual_, attr_);
   Client_SYS.Add_To_Attr('SIMULATION_VOUCHER', simulation_ , attr_);
   Client_SYS.Add_To_Attr('BALANCE', bal_, attr_);
...
IS
   ...
   CURSOR check_exist IS
      ...
      WHERE  EXISTS (SELECT * FROM company_finance_auth1 c WHERE v.company = c.company)
