...
BEGIN
   IF posting_type_ IS NOT NULL THEN
      Posting_Ctrl_API.Get_Override_Rec( override_rec_,
                                         company_,
                                         posting_type_,
                                         voucher_date_ );
   END IF;
   ...
   IF (need_to_validate_) THEN
      ...
      IF (project_origin_ = 'PROJECT') THEN
         $IF Component_Proj_SYS.INSTALLED $THEN            
            ...
            ELSE
               -- function group = 'P'
               state_ := PROJECT_API.Get_State(project_id_);
...
BEGIN
   ...
   -- Bug 101320, Begin, Using a different method in Account_API (newly created)
...
BEGIN
   ...
   WHILE Client_SYS.Get_Next_From_Attr(codestring_rec_, ptr_, name_, value_) LOOP
      ...
      ELSIF (name_ = 'PROCESS_CODE') THEN
         process_code_ := value_;
      ELSIF (name_ = 'TEXT') THEN      -- JPS
         text_ := value_;
      ELSIF (name_ = 'QUANTITY') THEN  -- JPS
         quantity_ := value_;
      END IF;
   END LOOP;
   codestr_rec_.code_a := code_part_a_;
   ...
   codestr_rec_.code_j := code_part_j_;
   codestr_rec_.process_code := process_code_;
   codestr_rec_.text := text_;         -- JPS
   codestr_rec_.quantity := quantity_; -- JPS
...
PROCEDURE Validate_Codeparts (
   ...
   validate_gl_rows_          BOOLEAN DEFAULT FALSE)
IS
   -- Bug 101320, moved into Impl-method
BEGIN
   
   -- Bug 101320, Begin, used Impl-method instead
   Validate_codeparts___( codestring_rec_, company_, voucher_date_,
                          check_codeparts_,
                          check_demands_codeparts_,
      ...
      IF (codepart_ IS NOT NULL) THEN
         IF (req_type_ = 'M') THEN
            Error_SYS.appl_general(lu_name_, 'WRONGREQMANDATORY: Account :P1 needs a value for code part :P2 :P3 according to budget code part demands.',
                                   codestring_rec_.code_a, codepart_, ACCOUNTING_CODE_PARTS_API.Get_Name(company_,codepart_));
         ...
         END IF;
      ELSE
         Error_SYS.appl_general(lu_name_,'WRONGREQ1: Wrong requirements for account :P1',
                    codestring_rec_.code_a);
      END IF;

   WHEN code_part_not_found THEN
      Error_SYS.appl_general(lu_name_,'CODEMISSING: :P1 :P2 is missing or has invalid time interval',
                 ACCOUNTING_CODE_PARTS_API.Get_Name(company_,code_part_), code_part_value_);
...
IS
BEGIN
   --Bug 101320, adding pragma to the method and remove General_SYS call
   ...
   $IF Component_Fixass_SYS.INSTALLED $THEN
      IF (Acquisition_Account_API.Is_Acquisition_Account (company_,
         ...
         IF (object_id_ IS NOT NULL) THEN
              ...
              object_state_ := FA_Object_API.Get_Object_state_Db(company_ , object_id_);
         ...
         END IF;
      END IF ;
      -- Bug 122655, Removed code from here and moved it outside the the condition
   $ELSE
      NULL;
   $END
   -- Bug 122655, Begin
...
EXCEPTION
   WHEN invalid_code_part THEN
      Error_SYS.appl_general(lu_name_,
                             'CODEMISSING: :P1 :P2 is missing or has invalid time interval',
                             ACCOUNTING_CODE_PARTS_API.Get_Name(company_, inval_code_part_),
                             code_part_value_);
   WHEN OTHERS THEN
      RAISE;
...
BEGIN
   ...
   hade user id as input parameter insted of user_group.
   */
   -- Bug 105357, Begin Added boolean flag to avoid Combination rule validation for BUDGET_DRIVER
