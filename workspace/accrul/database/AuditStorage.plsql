      ...
      ELSIF (name_arr_(n_) = 'NUMBER_OF_DAYS') THEN
         ...
         IF (days_checked_ = 'TRUE') THEN
            IF (number_of_days_ IS NULL) THEN
               Error_SYS.Item_General(lu_name_, 'NUMBER_OF_DAYS', 'NULLVALUE: Field [Days Before Current Date] is mandatory and requires a value.');
            END IF;
      ...
      ELSIF (name_arr_(n_) = 'BEFORE_DATE') THEN
         ...
         IF (days_checked_ = 'FALSE') THEN
            IF (before_date_ IS NULL) THEN
               Error_SYS.Item_General(lu_name_, 'BEFORE_DATE', 'NULLVALUE: Field [Created Before] is mandatory and requires a value.');
            END IF;
