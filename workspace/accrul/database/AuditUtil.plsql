...
--  160316  Nudilk  Bug 127779, Corrected to Remove New line charactors from data values.
--  160426  Nudilk  Bug 128584, Corrected.
-----------------------------------------------------------------------------
...
BEGIN
   ...
   -- If no translation exist for the given language, try to get the prog text
...
BEGIN
   ...
   -- Bug 107064, begin, Replaced non ASCII characters with ASCIISTR
   --                    equivalents.
...
BEGIN
   ...
   audit_format_        := Audit_Format_API.Get(company_, country_db_);
...
FUNCTION Get_Audit_Source_Desc___(
   audit_source_    IN VARCHAR2,
   audit_type_db_   IN VARCHAR2) RETURN VARCHAR2
IS 
   temp_       VARCHAR2(200);
   type_       VARCHAR2(50);
   path_       VARCHAR2(500);
...
BEGIN
   IF audit_type_db_ = 'REPORT' THEN
      type_ := 'Report';
      path_ := Report_SYS.Get_Lu_Name(audit_source_) || '.' || audit_source_;
      attribute_ := 'Title';
   ELSIF audit_type_db_ = 'VIEW' THEN
      RETURN Dictionary_SYS.Get_View_Prompt_(audit_source_);
      /*
      type_ := 'Column';
      path_ := Report_SYS.Get_Lu_Name(audit_source_) || '.' || audit_source_;
      attribute_ := 'Title';
...
BEGIN
   ...
   Generate_Output___ (objid_, country_, view_, company_, date_from_, date_to_, 'TRUE');
...
BEGIN
   ...
   IF batch_ = 'FALSE' THEN
      Generate_Output___ (objid_, country_, view_, company_, date_from_, date_to_, 'FALSE');
   ELSE
      attr_ := NULL;
