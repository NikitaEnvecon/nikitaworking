...
--  160426  Nudilk Bug 128833, Corrected.
--  160830  AjPelk Merged LCS Bug 130758.
-----------------------------------------------------------------------------
...
IS
   ...
   ref_currency_code_ currency_type.ref_currency_code%TYPE;
   counter_           NUMBER := 0;
...
BEGIN
   ref_currency_code_ := Currency_Type_API.Get_Ref_Currency_Code (company_,
                                                                  currency_type_);
   ...
   IF (NVL(Company_Finance_API.Get_Parallel_Base_Db(company_), ' ') = 'TRANSACTION_CURRENCY') THEN
      IF (Currency_Type_API.Get_Rate_Type_Category_Db(company_, currency_type_) = 'PARALLEL_CURRENCY') THEN
         parallel_curr_ := TRUE;
      END IF;
   END IF;
   -- Bug 128833,end.
   ...
   --IF (base_curr_found_) THEN
      FOR get_rate_ IN get_ext_curr_rates LOOP -- from Ext_File_Trans_Tab
         ...
         IF (currency_code_exist%FOUND) THEN
            ...
            -- If Base Currency is present in Text File Then The Currency rate should be equal to one
            IF (get_rate_.currency_code = base_currency_)  THEN
               IF (get_rate_.currency_rate != 1) THEN
                  Error_Sys.Record_General(lu_name_, 'RATENOTONE: Currency Rate Should be One for Base Currency :P1 in the Currency Rate File :P2',base_currency_,file_name_);
               END IF;
            END IF;

            -- Bug 128833,begin,modified if condition.
            IF ((ref_currency_code_ = base_currency_) OR parallel_curr_) THEN   -- If base currency of company = base currency in text file
            ...
            ELSE
               ...
               OPEN get_comp_curr_rate (ref_currency_code_);        --from Ext_Currency
               ...
               ELSE
                  ...
                  CLOSE get_comp_curr_rate;
                  Error_Sys.Record_General(lu_name_, 'NOREFCURR: Reference Currency :P1 not found in the Currency Rate File :P2',ref_currency_code_,file_name_);
               END IF;
            END IF;
            -- Bug 128833,end.
            -- Bug 130758, Begin
            ...
            Client_SYS.Add_To_Attr('REF_CURRENCY_CODE', ref_currency_code_,      rate_attr_);
