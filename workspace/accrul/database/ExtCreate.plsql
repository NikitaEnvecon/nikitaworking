...
--  151216  Maaylk Bug 126290, Stopped taking the currency_rate and conv_factor from Currency_Rate_API. instead it will be calculated in voucher_util_pub_API.
--  160429  Savmlk Bug 128663,Added new method Process_Voucher_Rows_By_Item__() and modified Process_Voucher_Rows__() and Create_Voucher() methods.
--  160609  Maaylk Bug 129671, Called Tax_Direction_API.Encode() to set the tax_direction
-----------------------------------------------------------------------------
...
IS
   ...
   temp_row_                     NUMBER;
   tax_transaction_              ext_voucher_row_tab.tax_transaction%TYPE;
...
BEGIN   
   ...
   IF (NVL(tax_transaction_,'FALSE') = 'FALSE') THEN
   ...
   IF main_correction_= 'TRUE' THEN
      correction_new2_:= correction_new_;
   ELSE
   ...
   IF (NVL(correction_new2_,'N') = 'N') THEN
      voucher_row_rec_.amount := abs(amount_);
      voucher_row_rec_.currency_amount := abs(curr_amount_);
      -- Bug 103653, Moved third currency amount code after tax reduction
      voucher_row_rec_.third_currency_amount := abs(third_curr_amt_);
...
--Specification[10/7/98] : Validation Level In Check External Voucher
   ...
   -- Bug 121638, Begin, Changed the IF condition for third_curr_tax_base_amount_. 
   ...
   IF (voucher_row_rec_.optional_code IS NOT NULL) THEN
      Statutory_Fee_API.Get_Fee_Type(tax_type_, company_, voucher_row_rec_.optional_code);
   ELSE
      -- Bug 97225, Begin
      Account_Tax_Code_API.Get_Default_Tax_Code (tax_type_,
   ...
   IF (tax_type_ IS NOT NULL AND tax_type_ NOT IN ('CALCVAT')) THEN
   ...
   IF voucher_row_rec_.third_currency_amount IS NULL THEN
      -- Bug 103653, Begin - Moved third currency calculation code to here 
   ...
   END IF;
   --Bug 126290 Begin. Added transaction_date_ IS NOT NULL to condition
   ...
   $IF Component_Fixass_SYS.INSTALLED $THEN
      IF (Acquisition_Account_Api.Is_Acquisition_Account(company_, codestring_rec_.code_a ) ) THEN 
         ...
         IF ( object_id_ IS NOT NULL )THEN 
            ...
            Client_Sys.Add_To_Attr('KEYREF1', NVL(voucher_no_,fictive_voucher_no_), add_investment_attr_ );
...
-- Misc performance (some gets moved to here and just performed when new voucher_date)
      ...
      -- -- Bug Id 122608 End   
         is_base_emu_      := Currency_Code_Api.Get_Valid_Emu (company_,
                                                               base_currency_code_,
                                                               voucher_date_);
         third_currency_code_ := Company_Finance_Api.Get_Parallel_Acc_Currency (company_,
                                                                                voucher_date_);
