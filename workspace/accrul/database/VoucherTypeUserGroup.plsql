...
BEGIN
   ...
   Client_SYS.Add_To_Attr( 'DEFAULT_TYPE', Default_type_API.DECODE('N'), attr_ );
...
BEGIN
   ...
   IF (newrec_.default_type = 'N') THEN
      Check_If_Default_Exist__(newrec_.company,
...
BEGIN
   -- Bug 109947, Begin
   IF (Company_Finance_API.Get_Use_Vou_No_Period(company_) = 'TRUE') AND Check_Exist___ (company_, accounting_year_, user_group_, voucher_type_) THEN
      RETURN;          
   END IF; 
   -- Bug 109947, End   
   IF (function_group_ = '*') OR (function_group_ IS NULL) THEN
      FOR rec_ IN get_function_group LOOP
         ...
         Client_SYS.Add_To_Attr('AUTHORIZE_LEVEL', authorize_level_, attr_);
         Client_SYS.Add_To_Attr('DEFAULT_TYPE', default_type_, attr_);
...
BEGIN
   ...
   -- Bug 111221,Begin removed IF condition 
   CLOSE get_voutype;
   -- Bug 111221,End
   -- Bug 111221,Begin
   RETURN  vou_type_;
