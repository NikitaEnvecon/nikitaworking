...
--  160208  Nudilk  Bug 127043, Corrected in Copy_Details_Set_Up.
-----------------------------------------------------------------------------
...
BEGIN
   IF codestring_rec_.code_a IS NULL THEN
      ...
      IF (posting_valid_date_ IS NOT NULL AND posting_valid_date_ > voucher_date_ ) THEN
         ...
         Error_SYS.Record_General(lu_name_, 'ACCNTNOTVALID: Posting Control Type :P1 has an invalid time interval', posting_type_);
      END IF;
      CLOSE get_valid_date;
   END IF;
...
BEGIN
   super(attr_);
   Client_SYS.Add_To_Attr( 'OVERRIDE', Posting_Ctrl_Comp_Code_Str_API.Decode('N'), attr_ );
...
BEGIN
   ...
   IF newrec_.override = 'Y' AND  (newrec_.ctrl_type_category = 'PREPOSTING') THEN
      code_part_function_    := Accounting_Code_Parts_API.Get_Code_Part_Function(newrec_.company,newrec_.code_part);
      code_part_function_db_ := Accounting_Code_Part_Fu_API.Encode(code_part_function_);
      IF ((code_part_function_db_ = 'PRACC') OR (code_part_function_db_ = 'FAACC') OR (code_part_function_db_ = 'CONSOL')) THEN
...
PROCEDURE Modify__ (
   ...
   action_     IN     VARCHAR2 )
IS
   oldrec_ POSTING_CTRL_TAB%ROWTYPE;
   newrec_ POSTING_CTRL_TAB%ROWTYPE;
BEGIN
   super(info_, objid_, objversion_, attr_, action_);
   Client_SYS.Add_To_Attr('CTRL_TYPE_CATEGORY_DB',newrec_.ctrl_type_category, attr_);
         ...
         IF (ctrl_type_category_ != 'PREPOSTING') THEN
            IF (code_part_ IS NULL) OR ( (code_part_value2_ IS NULL) AND (code_part_value_ IS NULL) ) THEN
               ...
               ELSIF (control_type_ = 'IC8') THEN
                  stmt_ := 'BEGIN :control_type_value_ := Type1099_API.Get_Irs1099_Type_Id(:control_type_value_); END;';
                  @ApproveDynamicStatement(2005-12-05,ovjose)
                  EXECUTE IMMEDIATE stmt_ using IN OUT control_type_value_;
               END IF;
...
BEGIN
   FOR cursor_rec IN override LOOP
      override_  := Posting_Ctrl_Comp_Code_Str_API.Decode(cursor_rec.override);
      IF cursor_rec.code_part = 'A' THEN
         override_rec_.code_a := override_;
      ELSIF cursor_rec.code_part = 'B' THEN
         override_rec_.code_b := override_;
      ELSIF cursor_rec.code_part = 'C' THEN
         override_rec_.code_c := override_;
      ELSIF cursor_rec.code_part = 'D' THEN
         override_rec_.code_d := override_;
      ELSIF cursor_rec.code_part = 'E' THEN
         override_rec_.code_e := override_;
      ELSIF cursor_rec.code_part = 'F' THEN
         override_rec_.code_f := override_;
      ELSIF cursor_rec.code_part = 'G' THEN
         override_rec_.code_g := override_;
      ELSIF cursor_rec.code_part = 'H' THEN
         override_rec_.code_h := override_;
      ELSIF cursor_rec.code_part = 'I' THEN
         override_rec_.code_i := override_;
      ELSIF cursor_rec.code_part = 'J' THEN
         override_rec_.code_j := override_;
      END IF;
   END LOOP;
...
BEGIN
   ...
   IF (get_default_value%NOTFOUND) OR (default_value_ IS NULL) THEN
...
BEGIN
   ...
   INSERT
      ...
      VALUES(
         ...
         override_,
         default_value_,
         Ctrl_Type_Category_API.Encode(Posting_Ctrl_Control_Type_API.Get_Ctrl_Type_Category(control_type_,
                                                                                            module_)),
         sysdate);
...
IS
   ...
   CURSOR getctrltype IS
      SELECT control_type FROM POSTING_CTRL_TAB
      WHERE  company      = company_
      AND    posting_type = posting_type_;
...
IS
BEGIN
   Posting_Ctrl_Posting_Type_API.Modify_Posting_Type_ ( posting_type_, description_,
                                                        module_ );
...
BEGIN
   FOR cursor_rec IN override LOOP
      IF cursor_rec.code_part = 'A' THEN
         override_rec_.code_a := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'B' THEN
         override_rec_.code_b := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'C' THEN
         override_rec_.code_c := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'D' THEN
         override_rec_.code_d := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'E' THEN
         override_rec_.code_e := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'F' THEN
         override_rec_.code_f := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'G' THEN
         override_rec_.code_g := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'H' THEN
         override_rec_.code_h := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'I' THEN
         override_rec_.code_i := cursor_rec.override;
      ELSIF cursor_rec.code_part = 'J' THEN
         override_rec_.code_j := cursor_rec.override;
      END IF;
   END LOOP;
  ...
  FUNCTION Get_Code_Part_Exist_ (
   ...
   BEGIN
      IF control_type_ IS NULL THEN
         RETURN 1;
      ELSE
         -- Bug 108452, Begin - Added nvl(pc_valid_from_ to last parameter
...
BEGIN
   ...
   WHILE(TRUE) LOOP
      ...
      IF code_part_ = 'A' THEN
         account_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'B' THEN
        code_b_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'C' THEN
         code_c_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'D' THEN
        code_d_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'E' THEN
        code_e_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'F' THEN
         code_f_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'G' THEN
         code_g_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'H' THEN
         code_h_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'I' THEN
         code_i_ := Get_Code_Part_Exist_(code_part_);
      ELSIF code_part_ = 'J' THEN
         code_j_ := Get_Code_Part_Exist_(code_part_);
      END IF;
   END LOOP;
...
BEGIN
   ...
   IF (get_posting%NOTFOUND) THEN
      override_iid_ := POSTING_CTRL_COMP_CODE_STR_API.Decode(override_);
      ...
      Client_SYS.Add_To_Attr('OVERRIDE', override_iid_, attr_);
      Client_SYS.Add_To_Attr('DEFAULT_VALUE', default_value_, attr_);
      Client_SYS.Add_To_Attr('DEFAULT_VALUE_NO_CT', default_value_no_ct_, attr_);
      Client_SYS.Add_To_Attr('CTRL_TYPE_CATEGORY', 
...
BEGIN
   ...
   IF NOT Check_Exist___(company_, code_part_, posting_type_, tmp_pc_valid_from_) THEN
      RETURN;
   END IF;
   ...
   IF (Company_Finance_API.Is_User_Authorized(company_ )) THEN
      ...
      IF (spec_control_type_ IS NOT NULL) THEN
         Client_SYS.Add_To_Attr('SPEC_CTRL_TYPE_CATEGORY', 
                                Get_Ctrl_Type_Category___(company_, 
                                                          posting_type_, 
                                                          spec_control_type_, 
                                                          code_part_), 
...
BEGIN
   ...
   UPDATE posting_ctrl_detail_tab
      ...
      OR    (spec_control_type = old_control_type_ AND spec_module = old_module_ AND (spec_ctrl_type_category IN ( 'ORDINARY', 'SYSTEM_DEFINED'))));
...
BEGIN
   -- posting_ctrl_detail_spec_tab
         DELETE FROM posting_ctrl_detail_spec_tab t		
    ...
    -- posting_ctrl_comb_det_spec_tab
    DELETE FROM posting_ctrl_comb_det_spec_tab t
                 WHERE  company = decode(company_, NULL, company, company_)
       AND    posting_type = posting_type_
              AND    code_part = decode(code_part_, NULL, code_part, code_part_)
       AND    EXISTS (SELECT 1
    ...
    -- posting_ctrl_detail_tab
    DELETE FROM posting_ctrl_detail_tab
       WHERE company = decode(company_, NULL, company, company_)
       AND   posting_type = posting_type_
       AND   code_part = decode(code_part_, NULL, code_part, code_part_)
       AND   control_type = control_type_
    ...
    -- posting_ctrl_comb_detail_tab
         DELETE FROM posting_ctrl_comb_detail_tab t
                 WHERE company      = decode(company_, NULL, company, company_)
                 AND   posting_type = posting_type_
                 AND   code_part    = decode(code_part_, NULL, code_part, code_part_)
       AND   EXISTS (SELECT 1
...
BEGIN
   ...
   FOR i_ IN 1..10 LOOP
      max_date_ := Get_Max_Date___(company_,
                                   ...
                                   CHR(64+i_),
                                   TRUNC(SYSDATE));
      IF ((NVL(Get_Ctrl_Type_Category_Db(company_, CHR(64+i_), posting_type_, max_date_),'COMBINATION') != 'COMBINATION') AND
          (Exist_Comb_In_Detail(company_, posting_type_, CHR(64+i_), max_date_) != 'TRUE')) THEN
