...
// 15-08-06    NUDILK    Bug 123877, Added static method RemoveBomCharactorFromString.
// 15-08-21    CLSTLK    Bug 123188, Modified SetCodepartVisibilityInProfile() to handle sVisibilityAtStartArray[] when profile has been reset.
// 15-08-24    CLSTLK    Bug 121843, Modified SetCodepartVisibilityInProfile() to handle visibility of code part columns without description.
// 15-10-08    Chwilk    Bug 124957, Modified RemoveBomCharactorFromString().
#endregion
...
namespace Ifs.Application.Accrul
{
	...
	public class cFinlibServices : cCalculateFinAmount
	{
		...
		public virtual SalNumber SetCodepartVisibilityInProfile()
        {
            ...
            #region Actions
            // Bug 121843,begin
            if (((bool)(nEndLoc - nStartLoc + 1 == 20) || (nEndLoc - nStartLoc + 1 == 10)) && __sProfileSection != Ifs.Fnd.ApplicationForms.Const.strNULL) 
			{
				Ifs.Fnd.ApplicationForms.Var.Profile.ValueStringGet(__sProfileSection, "ColVisibilityFin", Ifs.Fnd.ApplicationForms.Const.strNULL, ref __sVisibility);
				if (__sVisibility == Ifs.Fnd.ApplicationForms.Const.strNULL) 
				{
				...
				if (sCurrentVisibility != __sVisibilityAtStart) 
				{
                    ...
                    {
                        // Bug 121843,begin
                        // code part with description columns will be looped twice.
                        for (int i = 0; i < (nEndLoc - nStartLoc + 1) / 10; i++)
                        {
                            // Bug 123188, Begin Modified If condition to check sVisibilityAtStartArray[] when profile has been reset
                            if ((sVisibilityAtStartArray[nPosition] == "1" || sVisibilityAtStartArray[nPosition] == Sys.STRING_Null) && sCurrentVisibilityArray[nPosition] == "0")
                            {
                            ...
                            else if ((sVisibilityAtStartArray[nPosition] == "0" || sVisibilityAtStartArray[nPosition] == Sys.STRING_Null) && sCurrentVisibilityArray[nPosition] == "1") 
                            {
                        ...
                        // Bug 121843,End
                        // Bug 121843. Removed code which handled code part description
					}
        ...
        // Bug 123877,begin.
        // Note: This function remove UTF-8 BOM charactor from a given string.
        public static void RemoveBomCharactorFromString(ref SalString str)
        {
            string ByteOrderMarkUtf8 = Encoding.UTF8.GetString(Encoding.UTF8.GetPreamble());
            // Bug 124957, Begin
            SalString sHeader;
            // Bug 124957, End
            if (!string.IsNullOrEmpty(ByteOrderMarkUtf8))
            {
                // Bug 124957, Begin, Changed condition
                sHeader = str.ToString().Substring(0, ByteOrderMarkUtf8.Length);

                if (sHeader.ToString().Equals(ByteOrderMarkUtf8))
                {
                    str = str.ToString().Remove(0, ByteOrderMarkUtf8.Length);
                }
                // Bug 124957, End
            }
