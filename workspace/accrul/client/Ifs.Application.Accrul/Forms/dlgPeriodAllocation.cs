...
// 2015-08-20  THPELK    FISU-275, Corrected.
...
namespace Ifs.Application.Accrul
{
	...
	public partial class dlgPeriodAllocation : cDialogBoxFin
	{
        ...
        public SalString sIsCancellation;
		#endregion
		...
		/// <returns></returns>
        public static SalNumber ModalDialog(Control owner, SalString sPCompany, SalNumber nPAccountingPeriod, SalString sIsCancellation)
		{
            ...
            dlg.sIsCancellation = sIsCancellation;            
		...
		public new SalBoolean FrameStartupUser()
		{
			...
			using (new SalContext(this))
			{
                ...
                if (sIsCancellation == "TRUE")
                {
                    ...
                    Sal.DisableWindow(pbCancel);
                    Sal.DisableWindow(pbNew);
                    Sal.DisableWindow(pbRemove);
                    Sal.DisableWindow(pbList);
                    Sal.DisableWindow(pbSave);

                    Sal.DisableWindow(tblPeriod_colsCompany);
                    Sal.DisableWindow(tblPeriod_colsVoucherType);
                    Sal.DisableWindow(tblPeriod_colnVoucherNo);
                    Sal.DisableWindow(tblPeriod_colnAccountingYear);
                    Sal.DisableWindow(tblPeriod_colnRowNo);
                    Sal.DisableWindow(tblPeriod_colnAllocPeriod);
                    Sal.DisableWindow(tblPeriod_colCompletePeriod);
                    Sal.DisableWindow(tblPeriod_colnAllocPercent);
                    Sal.DisableWindow(tblPeriod_colnAllocAmount);
                    Sal.DisableWindow(tblPeriod_colnAllocYear);
                    Sal.DisableWindow(tblPeriod_colnUntilYear);
                    Sal.DisableWindow(tblPeriod_colnUntilPeriod);
                    Sal.DisableWindow(tblPeriod_colnUserGroup);
                    Sal.DisableWindow(tblPeriod_colnAllocVouType);                  
