...
//  16-01-14  Savmlk  Bug 126734, Fixed.
//  16-09-05  Dakplk  Bug 131287, Modified the initialization of the variable 'sCheckDuplicate'
#endregion
...
namespace Ifs.Application.Accrul
{
    ...
    public partial class frmTabAccount : cTabFormWindowFin
    {
        ...
        private SalString sRepBalExist = "FALSE";
        //Bug 126734, Begin
        //Bug 131287, Begin, Modified the initialization
        public SalString sCheckDuplicate = "FALSE";
        //Bug 131287, End
        //Bug 126734, End
        ...
        protected virtual void GetReportedBalancesExist()
        {
            #region Actions
            if (Ifs.Fnd.ApplicationForms.Int.IsSystemComponentInstalled("GROCON"))
            {
                DbPLSQLBlock(":i_hWndFrame.frmTabAccount.sRepBalExist := &AO.Reported_Balances_Details_API.Rep_Bal_Exist(:i_hWndFrame.frmTabAccount.dfsCompany IN ,:i_hWndFrame.frmTabAccount.cmbAccount IN);");
            }
            #endregion
        }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual SalNumber EnableDisableGroconInfo()
        {
            #region Actions

            if (Ifs.Fnd.ApplicationForms.Int.IsSystemComponentInstalled("GROCON") && Ifs.Application.Accrul.Var.FinlibServices.IsMasterCompany(i_sCompany) == "TRUE")
            {
                Sal.EnableWindowAndLabel(gbGroupConsolidation);
                Sal.EnableWindowAndLabel(cbExcludeCurrTrans);
                Sal.EnableWindowAndLabel(cbKeepRepCurrency);
                Sal.EnableWindowAndLabel(cbKeepRepCurrency);
            }
            else
            {
                Sal.EnableWindowAndLabel(gbGroupConsolidation);
                Sal.DisableWindowAndLabel(cbExcludeCurrTrans);
                Sal.DisableWindowAndLabel(cbKeepRepCurrency);
                Sal.DisableWindowAndLabel(cbKeepRepCurrency);
            }              
        ...
        private void dfsAccountType_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if (this.dfsAccountType.Text != Ifs.Fnd.ApplicationForms.Const.strNULL && Sal.SendMsg(this.dfsAccountType.i_hWndSelf, Ifs.Fnd.ApplicationForms.Const.PM_DataItemQueryEnabled, 0, 0) == Ifs.Fnd.ApplicationForms.Const.EDITSTATEFieldEnabled)
                {
                    ...
                    //Bug 126734, Begin - Added new if condition.
                    if (sCheckDuplicate == "FALSE")
                    {
        ...
        private void cbKeepRepCurrency_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Click:
                    this.cbKeepRepCurrency_OnSamClick(sender, e);
                    break;
            }
            #endregion            
        ...
        /// </summary>
        /// <param name="message"></param>
        private void cbKeepRepCurrency_OnSamClick(object sender, WindowActionsEventArgs e)
        {
            #region Local Variables
            SalNumber nChoice = Sys.NUMBER_Null;            
            #endregion
            ...
            if (Sal.SendClassMessage(Sys.SAM_Click, Sys.wParam, Sys.lParam))
            {                
                GetReportedBalancesExist();
                if (sRepBalExist == "TRUE")
                {
                    nChoice = Ifs.Fnd.ApplicationForms.Int.AlertBox(Properties.Resources.TEXT_AccountExistRepBal, Ifs.Fnd.ApplicationForms.Properties.Resources.CAPTION_Question, (Ifs.Fnd.ApplicationForms.Const.QUESTION_YesNo | Sys.MB_DefButton2));
                    ...
                    {
                        if (cbKeepRepCurrency.Checked)
                        {
                            cbKeepRepCurrency.Clear();
                        }
                        else
                        {
                            cbKeepRepCurrency.Checked = true;
                        }
                        e.Return = false;
                        return;
                    }
        ...
        private void cbKeepReportingEntity_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Click:
                    this.cbKeepReportingEntity_OnSamClick(sender, e);
                    break;
            }
            #endregion     
        }
        ...
        /// </summary>
        /// <param name="message"></param>
        private void cbKeepReportingEntity_OnSamClick(object sender, WindowActionsEventArgs e)
        {
            #region Local Variables
            SalNumber nChoice = Sys.NUMBER_Null;            
            #endregion
            ...
            if (Sal.SendClassMessage(Sys.SAM_Click, Sys.wParam, Sys.lParam))
            {
                ...
                if (sRepBalExist == "TRUE")
                {
                    ...
                    {
                        if (cbKeepReportingEntity.Checked)
                        {
                            cbKeepReportingEntity.Clear();
                        }
                        else
                        {
                            cbKeepReportingEntity.Checked = true;
                        }
                        e.Return = false;
                        return;
                    }
