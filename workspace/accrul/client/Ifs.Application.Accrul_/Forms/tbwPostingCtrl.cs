...
namespace Ifs.Application.Accrul_
{
    ...
    public partial class tbwPostingCtrl : cTableWindowFin
    {
        ...
        private void colsCodeName_WindowActions(object sender, WindowActionsEventArgs e)
        {
            ...
            switch (e.ActionType)
            {
                ...
                case Ifs.Fnd.ApplicationForms.Const.PM_DataItemLovDone:
                    this.colsCodeName_OnPM_DataItemLovDone(sender, e);
                    break;
                // Bug 129199,end.
        ...
        // Bug 129199,begin.
        private void colsCodeName_OnPM_DataItemLovDone(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.nNum = SalString.FromHandle(Sys.lParam).Tokenize("", ((SalNumber)Ifs.Fnd.ApplicationForms.Const.CHAR_US).ToCharacter(), this.sUnits);
            if (this.sUnits[0] == "CODE_PART")
                this.colsCodePart.Text = this.sUnits[1];
            if (this.sUnits[2] == "CODE_NAME")
                this.colsCodeName.Text = this.sUnits[3];
            #endregion
        }
        // Bug 129199,end.
        ...
        private void colsCodeName_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            {
                // Bug 129199,begin.
                if (string.IsNullOrEmpty(this.colsCodePart.Text))
                {
                    using (SignatureHints hints = SignatureHints.NewContext())
                    {
