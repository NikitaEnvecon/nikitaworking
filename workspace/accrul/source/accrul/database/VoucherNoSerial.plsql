...
--  151211  CHWTLK   Bug-126216, Modified Check_Update___();
--  150217  SAVMLK   Bug 127361, Corrected.
      ...
      IF (Company_Finance_API.Get_User_Def_Cal(crecomp_rec_.company) = 'FALSE') THEN
         IF (Company_Finance_API.Get_Use_Vou_No_Period(crecomp_rec_.company) = 'FALSE') THEN
            FOR rec_ IN get_data LOOP
               ...
               BEGIN
                  Add_Serie___(objid_, objversion_, attr_, crecomp_rec_.company,
                               rec_.c1, rec_.n1, rec_.n2, rec_.n3, rec_.n2, 99);
                  data_found_ := TRUE;
               EXCEPTION
                  WHEN OTHERS THEN
                     msg_ := SQLERRM;
            ...
            IF (crecomp_rec_.user_defined = 'TRUE') THEN
               ...
               FOR rec_ IN get_distinct_voucher_type LOOP
                  FOR year_ IN crecomp_rec_.acc_year..end_year_ LOOP
                     BEGIN
                        Add_Serie___(objid_, objversion_, attr_,
                                     crecomp_rec_.company, rec_.c1, year_,
...
BEGIN
   ...
   IF (crecomp_rec_.user_defined = 'TRUE' AND Company_Finance_API.Get_Use_Vou_No_Period(crecomp_rec_.company) = 'TRUE') THEN
   ...
   ELSIF (crecomp_rec_.user_defined = 'FALSE' AND 
          ...
          Company_Finance_API.Get_Use_Vou_No_Period(crecomp_rec_.company) = 'TRUE') THEN
      FOR get_year_ IN get_year LOOP
         serei_from_ := TO_NUMBER(TO_CHAR(get_year_.accounting_year)||'000001');
      ...
      IF (NOT any_rows_) AND
         ...
         FOR get_voucher_types_ IN get_voucher_types LOOP
            FOR year_ IN 0..crecomp_rec_.number_of_years-1 LOOP
               ...
               BEGIN
                   Add_Serie___(objid_, objversion_, attr_,
                   crecomp_rec_.company, get_voucher_types_.voucher_type, acc_year_,
...
BEGIN
   ...
   serei_from_ := TO_NUMBER(TO_CHAR(year_)||'000001');
               ...
               FOR get_voucher_types_ IN get_voucher_types_comp_ LOOP
                  ...
                  BEGIN
                     IF NOT Check_Exist___(company_, get_voucher_types_.voucher_type, year_, period_) THEN
                        Add_Serie___(objid_, objversion_, attr_, company_,
                                     get_voucher_types_.voucher_type, year_, serei_from_,
               ...
               FOR get_voucher_types_ IN get_voucher_types LOOP
                  ...
                  BEGIN
                     IF NOT Check_Exist___(company_, get_voucher_types_.c1, year_, period_) THEN
                        Add_Serie___(objid_, objversion_, attr_, company_,
                                     get_voucher_types_.c1, year_, serei_from_,
                                     serei_until_, serei_from_, period_);
                     END IF;
                  EXCEPTION
                     WHEN OTHERS THEN
...
BEGIN
   ...
   IF (ledger_id_ NOT IN ( '*','00')) THEN
      -- Bug 126216, Begin, Included newrec_.accounting_year as a parameter.
   ...
   -- Bug Id 125704 begin Change the IF condition
...
EXCEPTION
   WHEN current_no_exp THEN
      Client_SYS.Clear_Attr(attr_);
      -- Bug Id 125704 begin  Change the newrec_ to oldrec_
...
BEGIN
   ...
   IF (Company_Finance_API.Get_Use_Vou_No_Period(company_) = 'FALSE') THEN
      period_ := 99;
   ELSE 
      -- Bug 108867, Begin - Modified the IF Condition 
   ...
   IF (get_voucher_number%NOTFOUND) THEN
      ...
      voucher_no_ := 0;
      Error_SYS.Record_General(lu_name_,'NO_SER: There are no voucher no series for this voucher type and accounting_year');
   ELSE
      CLOSE get_voucher_number;
   END IF;
...
IS
   objid_               VOUCHER_NO_SERIAL.objid%TYPE;
   objversion_          VARCHAR2(2000);
   info_                VARCHAR2(1000);
   -- Bug 127361, Begin
   attr_                VARCHAR2(8000);
...
BEGIN
            ...
            FETCH get_acc_year INTO acc_year_;
               ...
               IF(voucher_per_used_ = 'TRUE') THEN
                  ...
                  LOOP
                     ...
                     IF (vou_serial%FOUND) THEN
                        ...
                        Client_SYS.Add_To_Attr('SERIE_FROM', serie_from_, attr_);
                        Client_SYS.Add_To_Attr('SERIE_UNTIL', serie_until_, attr_);
                        current_number_ := serie_from_;
                        Client_SYS.Add_To_Attr('CURRENT_NUMBER', current_number_, attr_);
                   ...
                   FOR rec_func_goup_ IN get_function_group LOOP
                      ...
                      FOR rec_user_gr_func_gr_ IN usr_grp_dat_per_func_grp LOOP
                         ...
                         voucher_Type_User_Group_Api.Create_Vou_Type_User_Group(company_,
                                                                                ...
                                                                                Default_Type_API.Decode(def_default_type_),
                                                                                func_group_ );
                      END LOOP;
               ...
               ELSE
                  ...
                  IF (vou_serial%FOUND) THEN
                     ...
                     Client_SYS.Add_To_Attr('VOUCHER_TYPE', voucher_type_, attr_);
                     Client_SYS.Add_To_Attr('ACCOUNTING_YEAR', acc_year_, attr_);
                     Client_SYS.Add_To_Attr('PERIOD', 99, attr_);     
                     Client_SYS.Add_To_Attr('SERIE_FROM', serie_from_, attr_);
                     Client_SYS.Add_To_Attr('SERIE_UNTIL', serie_until_, attr_);
                     current_number_ := serie_from_;
                     Client_SYS.Add_To_Attr('CURRENT_NUMBER', current_number_, attr_);
   ...
   ELSE
      ...
      FOR i_ IN 1..count_ LOOP
         ...
         Client_SYS.Add_To_Attr('VOUCHER_TYPE',    rec_.voucher_type,    attr_);
         Client_SYS.Add_To_Attr('ACCOUNTING_YEAR', rec_.accounting_year, attr_);
         Client_SYS.Add_To_Attr('PERIOD',          rec_.period,          attr_);
         Client_SYS.Add_To_Attr('SERIE_FROM',      rec_.serie_from,      attr_);
         Client_SYS.Add_To_Attr('SERIE_UNTIL',     rec_.serie_until,     attr_);
         Client_SYS.Add_To_Attr('CURRENT_NUMBER',  rec_.current_number,  attr_);
