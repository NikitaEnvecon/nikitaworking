...
--  160429  Savmlk Bug 128663,Added new method Update_Errors().
-----------------------------------------------------------------------------
...
-- Bug 128663, Begin- This method update all voucher errors for one load group.
...
BEGIN
   IF (auto_tax_calc_ = 'Y') THEN
      ...
      IF transaction_rec_.optional_code IS NULL THEN
         -- Bug 97225, Begin
         Account_Tax_Code_API.Get_Default_Tax_Code (transaction_rec_.optional_code,
                                                    transaction_rec_.tax_direction,
                                                    transaction_rec_.company,
                                                    transaction_rec_.codestring_rec.code_a,
      ...
      IF (transaction_rec_.optional_code IS NOT NULL) THEN
         IF (transaction_rec_.optional_code IS NOT NULL AND
             transaction_rec_.tax_amount IS NULL AND
             transaction_rec_.currency_tax_amount IS NULL) THEN
            transaction_rec_.tax_direction := Tax_Direction_API.Encode(transaction_rec_.tax_direction);
            -- Bug 97225, Begin
            Statutory_Fee_API.Get_Tax_Code_Info(multiple_tax_, tax_type_, deductible_percent_,transaction_rec_.company, transaction_rec_.optional_code);  
            ...
            fee_percent_ := Statutory_Fee_API.Get_Calculated_Percent(transaction_rec_.company,
                                                                     tax_type_,
                                                                     transaction_rec_.optional_code,
                                                                     transaction_rec_.load_date);
            ...
            IF (NVL(fee_percent_,0) != 0) THEN
               ...
               IF (tax_type_ NOT IN ('CALCVAT')) THEN
                  ...
                  IF (def_amount_method_ = 'GROSS') THEN
                     total_currency_tax_amount_           := transaction_rec_.currency_amount * fee_percent_ / ( 1 + fee_percent_ );
                     total_tax_amount_                    := transaction_rec_.amount * fee_percent_ / ( 1 + fee_percent_ );
                     IF (deductible_percent_ != 1 AND check_deductible_ = 'TRUE')  THEN
                        transaction_rec_.currency_tax_amount := ROUND(total_currency_tax_amount_* deductible_percent_, currency_rounding_);                        
                        transaction_rec_.tax_amount          := ROUND(total_tax_amount_* deductible_percent_, rounding_);                        
                     ELSE
                        transaction_rec_.currency_tax_amount := ROUND(total_currency_tax_amount_, currency_rounding_);
                        transaction_rec_.tax_amount          := ROUND(total_tax_amount_, rounding_);
                     END IF;
                     transaction_rec_.third_currency_tax_amount  := ROUND((transaction_rec_.third_currency_tax_amount * fee_percent_ / ( 1 + fee_percent_ )), parallel_curr_rounding_);
                  ELSE
                     total_currency_tax_amount_           := transaction_rec_.currency_amount * fee_percent_;
                     total_tax_amount_                    := transaction_rec_.amount * fee_percent_;
                     IF (deductible_percent_ != 1 AND check_deductible_ = 'TRUE')  THEN
                        transaction_rec_.currency_tax_amount := ROUND(total_currency_tax_amount_ * deductible_percent_, currency_rounding_);
                        transaction_rec_.currency_amount     := ROUND(transaction_rec_.currency_amount + (total_currency_tax_amount_ - transaction_rec_.currency_tax_amount), currency_rounding_);
                        transaction_rec_.tax_amount          := ROUND((total_tax_amount_ * deductible_percent_), rounding_);
                        transaction_rec_.amount              := ROUND(transaction_rec_.amount + (total_tax_amount_ - transaction_rec_.tax_amount), rounding_);
                        -- Bug 97225, Begin
                        IF (transaction_rec_.debet_amount IS NOT NULL ) THEN
                           transaction_rec_.debet_amount := transaction_rec_.amount;                           
                        ELSIF (transaction_rec_.credit_amount IS NOT NULL ) THEN
                     ...
                     ELSE
                        ...
                        transaction_rec_.tax_amount            := ROUND(total_tax_amount_, rounding_);
					      END IF;
                     transaction_rec_.third_currency_tax_amount := ROUND((transaction_rec_.third_currency_amount * fee_percent_), parallel_curr_rounding_);
                  END IF;
                  -- Bug 97225, End
               ELSE
                  transaction_rec_.currency_tax_amount := ROUND((transaction_rec_.currency_amount * fee_percent_), currency_rounding_);
                  transaction_rec_.tax_amount          := ROUND((transaction_rec_.amount * fee_percent_), rounding_);
                  transaction_rec_.third_currency_tax_amount := ROUND((transaction_rec_.third_currency_amount * fee_percent_), rounding_);   
               END IF;
...
BEGIN
   FOR rec_ IN Voucher_Type LOOP
      ...
      IF (function_group_ = 'Q') THEN
         fun_group_q_exist_ := 'TRUE';
         EXIT;
      END IF;
      fun_group_q_exist_ := 'FALSE';
   END LOOP;
   RETURN fun_group_q_exist_;
