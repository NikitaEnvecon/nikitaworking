...
--  160526  CHWTLK   Bug 129417, Modified Check_Common___.
-----------------------------------------------------------------------------
...
BEGIN   
   ...
   IF (newrec_.inverted = 'TRUE') THEN
      -- Bug 129417, Begin, Restricted companies which don't use parallel currency to set inverted currency. Allow if its Accounting currency or EUR.
...
IS
   decimals_rate_    NUMBER;
BEGIN
   decimals_rate_  := Get_No_Of_Decimals_In_Rate(company_, currency_code_);
   IF (decimals_rate_ IS NULL) THEN
      Error_SYS.Appl_General(lu_name_,'NOCURR: The currency :P1 does not exist', currency_code_);
   END IF;
   decimals_in_rate_ := decimals_rate_;
