   ...
   FOR advsel_ IN get_advanced LOOP
      ...
      ELSE
         ...
         IF company_def_ THEN
            -- Bug 106065, Added bind variable.
            where_ :=  ' COMPANY = :company_' || ' AND ' || where_;
         END IF;
         
         -- Bug 106065, Changed the If statements to match with the execution of statement.
         IF (company_def_)THEN
...
BEGIN
   FOR obj_ IN get_obj LOOP
      IF Fin_Sel_Object_API.Get_Is_Code_Part(obj_.selection_object_id) = 'FALSE' THEN
         obj_selection_(obj_.selection_object_id).selection := NULL;
      ELSE
         code_part_ := Fin_Sel_Object_API.Get_Code_Part(obj_.selection_object_id);
   ...
   FOR advsel_ IN get_advanced LOOP
      ...
      IF Fin_Sel_Object_API.Get_Is_Code_Part(advsel_.selection_object_id) = 'FALSE' THEN
         sel_object_id_ := advsel_.selection_object_id;
      ELSE
         code_part_ := Fin_Sel_Object_API.Get_Code_Part(advsel_.selection_object_id);
      ...
      IF value_from_ IS NOT NULL THEN
         template_rec_(ctr_).selection_object := Fin_Sel_Object_API.Get_Description(company_, advsel_.selection_object_id);
