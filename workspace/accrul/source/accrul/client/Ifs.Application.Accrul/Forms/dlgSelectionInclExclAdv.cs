...
namespace Ifs.Application.Accrul
{
	...
	public partial class dlgSelectionInclExclAdv : cDialogBox
	{
		...
		/// Default Constructor.
		/// </summary>
		protected dlgSelectionInclExclAdv()
		{
		...
		/// <returns></returns>
		public static SalNumber ModalDialog(Control owner, SalString p_sCompany, SalString p_sObjectGroupId, SalString p_sTemplateId, SalNumber p_nItemId, SalString p_sView, SalString p_sColId, SalString p_sColDesc, SalString p_sUseCompany)
		{
			dlgSelectionInclExclAdv dlg = DialogFactory.CreateInstance<dlgSelectionInclExclAdv>();
		...
		/// <returns></returns>
		public new static dlgSelectionInclExclAdv FromHandle(SalWindowHandle handle)
		{
			return ((dlgSelectionInclExclAdv)SalWindow.FromHandle(handle, typeof(dlgSelectionInclExclAdv)));
		}
		#endregion
		...
		public new SalNumber UserMethod(SalNumber nWhat, SalString sMethod)
		{
			...
			using (new SalContext(this))
			{
				if (sMethod == "LOV") 
				{
					...
					{
						return Sal.SendMsg(this.tblValues_colValue, Ifs.Fnd.ApplicationForms.Const.PM_DataItemLov, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire, 0);
					}
					if (nWhat == Ifs.Fnd.ApplicationForms.Const.METHOD_Execute) 
					{
						Sal.SendMsg(this.tblValues_colValue, Ifs.Fnd.ApplicationForms.Const.PM_DataItemLov, Ifs.Fnd.ApplicationForms.Const.METHOD_Execute, 0);
						Sal.TblSetFocusCell(tblValues, Sal.TblQueryContext(tblValues), this.tblValues_colValue, 0, -1);
					}
				}
			}
		...
		private void dlgSelectionInclExclAdv_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Sys.SAM_Create:
                    this.dlgSelectionInclExclAdv_OnSAM_Create(sender, e);
					break;
			}
			#endregion
		}
		...
		private void dlgSelectionInclExclAdv_OnSAM_Create(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
		...
		private void tblValues_OnSAM_FetchRowDone(object sender, WindowActionsEventArgs e)
		{
			...
			// Call GetStatusAndDescription()
			if (this.tblValues_colDescription.Text != Ifs.Fnd.ApplicationForms.Const.strNULL) 
			{
				this.tblValues_colStatus.Text = Properties.Resources.TEXT_SEL_TEMPL_ArgumentExist;
			}
			else
			{
				this.tblValues_colDescription.Text = Ifs.Fnd.ApplicationForms.Const.strNULL;
				this.tblValues_colStatus.Text = Properties.Resources.TEXT_SEL_TEMPL_ArgumentNotExist;
			}
			#endregion
		}
		...
		public virtual new SalNumber DataRecordGetDefaults()
		{
			...
			tblValues_colTemplateId.Text = this.p_sTemplateId;
			...
			tblValues_colTemplateId.EditDataItemSetEdited();
		...
		public virtual SalNumber GetStatusAndDescription()
		{
			...
			using (new SalContext(tblValues))
			{
				...
				"    :i_hWndFrame.dlgSelectionInclExclAdv.sDescription \r\n" +
				" FROM &AO." + this.p_sView;
				if (this.p_sUseCompany == "TRUE") 
				{
					sSqlStatment = sSqlStatment + " WHERE COMPANY=:i_hWndFrame.dlgSelectionInclExclAdv.p_sCompany \r\n" +
					"        AND " + this.p_sColId + " = :i_hWndFrame.dlgSelectionInclExclAdv.tblValues_colValue";
				}
				else
				{
					sSqlStatment = sSqlStatment + " WHERE " + this.p_sColId + " = :i_hWndFrame.dlgSelectionInclExclAdv.tblValues_colValue";
				}
		...
		/// </summary>
		/// <returns></returns>
		public virtual new SalString DataSourceFormatSqlColumnUser()
		{
			SalString sCompanyWhere = "";
		    if (this.p_sUseCompany == "TRUE") 
		    {
			    sCompanyWhere = "a.company = " + tblValues.p_sViewName + ".company and ";
			    // Set sCompanyWhere = "a.company = "
		    }
		    return ("(select " + this.p_sColDesc + " from &AO." + this.p_sView + " a where " + sCompanyWhere + " a." + this.p_sColId + " = " + tblValues.p_sViewName + ".value)");
		...
		/// </summary>
		/// <returns></returns>
		public virtual new SalString DataSourceFormatSqlIntoUser()
		{
            return tblValues_colDescription.QualifiedBindName;
		}
		#endregion
        ...
        private void tblValues_DataSourceFormatSqlColumnUserEvent(object sender, FndReturnEventArgsSalString e)
        {
            e.Handled = true;
            e.ReturnValue = this.DataSourceFormatSqlColumnUser();
        }

        private void tblValues_DataSourceFormatSqlIntoUserEvent(object sender, FndReturnEventArgsSalString e)
        {
            e.Handled = true;
