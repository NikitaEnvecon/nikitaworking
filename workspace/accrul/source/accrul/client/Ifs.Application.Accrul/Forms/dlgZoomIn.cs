...
// Date    By      Notes
// -----  ------  -------------------------------------------------------------------------------------
//111230  Maaylk  Bug 97798, Added 2 check boxes cbIgnoreDescription, cbIgnoreOrderBy to dlgZoomIn
#endregion
...
namespace Ifs.Application.Accrul
{
	...
	public partial class dlgZoomIn : cDialogBoxFin
	{
		...
		/// <returns></returns>
		public static SalNumber ModalDialog(Control owner, SalString p_sCompany, SalWindowHandle p_hWndInvoker, SalArray<SalBoolean> p_bVisible, ref SalArray<SalBoolean> p_bCode)
		{
			dlgZoomIn dlg = DialogFactory.CreateInstance<dlgZoomIn>();
		...
		/// <returns></returns>
		public new static dlgZoomIn FromHandle(SalWindowHandle handle)
		{
			return ((dlgZoomIn)SalWindow.FromHandle(handle, typeof(dlgZoomIn)));
		}
		#endregion
		...
		public new SalBoolean FrameStartupUser()
		{
			...
			using (new SalContext(this))
			{
				...
				cbAccountType.Checked = p_bCode[2];
				cbAccountGroup.Checked = p_bCode[3];
				if (!(p_bVisible[0])) 
				{
					Sal.DisableWindow(cbAttribute);
				}
				else
				{
					cbAttribute.Checked = p_bCode[4];
				}
				cbAccount.Checked = p_bCode[5];
				if (!(p_bVisible[1])) 
				{
				...
				{
					cbCode_J.Checked = p_bCode[14];
				}
				// Simulation Voucher
				cbSimulation.Checked = p_bCode[15];
				cbNormal.Checked = p_bCode[16];
				if (!(p_bCode[15]) && !(p_bCode[16])) 
				{
					cbNormal.Checked = true;
				}
				cbIncludeHold.Checked = p_bCode[17];
				// Bug 97798, Begin
				cbIgnoreDescription.Checked = p_bCode[18];
				// Bug 97798, End
				return true;
			}
			#endregion
			...
			using (new SalContext(this))
			{
				...
				else if (nWhat == Ifs.Fnd.ApplicationForms.Const.METHOD_Execute) 
				{
					...
					if (p_bCode[0] == true) 
					{
						...
						// Bug 97798, Begin.
						if (cbIgnoreDescription.Checked) 
						{
							sCode = sCode + "^" + "1";
						}
						else
		...
		private void dlgZoomIn_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Sys.SAM_Create:
					this.dlgZoomIn_OnSAM_Create(sender, e);
					break;
			}
			#endregion
		}
		...
		private void dlgZoomIn_OnSAM_Create(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
