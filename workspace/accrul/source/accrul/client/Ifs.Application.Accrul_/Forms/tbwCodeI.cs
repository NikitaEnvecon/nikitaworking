...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_I,ACCOUNTING_CODEPART_I,CODE_I_ALL", "CodeI", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeI : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeI FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeI)SalWindow.FromHandle(handle, typeof(tbwCodeI)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeI;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeI", this, sArray, hWndItems);
        ...
        private void tbwCodeI_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeI_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeI_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeI_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_I";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeI_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeI_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeI.Text).Scan("&") >= 0) || (((SalString)this.colsCodeI.Text).Scan("'") >= 0) || (((SalString)this.colsCodeI.Text).Scan("^") >= 0))
                {
