...
namespace Ifs.Application.Accrul_
{
	...
	public partial class frmExternalFileParamSet : cFormWindow
	{
        ...
        public virtual SalNumber PopulateValues()
        {
            ...
            using (new SalContext(this))
            {
                nCountRow = Sal.TblSetRow(i_hWndSelf, Sys.TBL_SetLastRow);
                nCurrentRow = Sal.TblSetRow(i_hWndSelf, Sys.TBL_SetFirstRow);
                while (nCurrentRow <= nCountRow)
                {
                    if (Sal.TblSetContext(i_hWndSelf, nCurrentRow))
                    {
