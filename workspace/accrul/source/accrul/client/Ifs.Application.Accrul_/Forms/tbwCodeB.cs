...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_B,ACCOUNTING_CODEPART_B,CODE_B_ALL", "CodeB", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeB : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeB FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeB)SalWindow.FromHandle(handle, typeof(tbwCodeB)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeB;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeB", this, sArray, hWndItems);
        ...
        private void tbwCodeB_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeB_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeB_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeB_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_B";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeB_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeB_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeB.Text).Scan("&") >= 0) || (((SalString)this.colsCodeB.Text).Scan("'") >= 0) || (((SalString)this.colsCodeB.Text).Scan("^") >= 0))
                {
