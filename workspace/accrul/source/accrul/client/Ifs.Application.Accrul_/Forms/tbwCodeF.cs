...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_F,ACCOUNTING_CODEPART_F,CODE_F_ALL", "CodeF", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeF : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeF FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeF)SalWindow.FromHandle(handle, typeof(tbwCodeF)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeF;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeF", this, sArray, hWndItems);
        ...
        private void tbwCodeF_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeF_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeF_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeF_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_F";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeF_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeF_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeF.Text).Scan("&") >= 0) || (((SalString)this.colsCodeF.Text).Scan("'") >= 0) || (((SalString)this.colsCodeF.Text).Scan("^") >= 0))
                {
