...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_G,ACCOUNTING_CODEPART_G,CODE_G_ALL", "CodeG", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeG : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeG FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeG)SalWindow.FromHandle(handle, typeof(tbwCodeG)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeG;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeG", this, sArray, hWndItems);
        ...
        private void tbwCodeG_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeG_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeG_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeG_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_G";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeG_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeG_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeG.Text).Scan("&") >= 0) || (((SalString)this.colsCodeG.Text).Scan("'") >= 0) || (((SalString)this.colsCodeG.Text).Scan("^") >= 0))
                {
