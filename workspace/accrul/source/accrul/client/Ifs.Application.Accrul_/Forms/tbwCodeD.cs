...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_D,ACCOUNTING_CODEPART_D,CODE_D_ALL", "CodeD", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeD : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeD FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeD)SalWindow.FromHandle(handle, typeof(tbwCodeD)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeD;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeD", this, sArray, hWndItems);
        ...
        private void tbwCodeD_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeD_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeD_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeD_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_D";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeD_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeD_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeD.Text).Scan("&") >= 0) || (((SalString)this.colsCodeD.Text).Scan("'") >= 0) || (((SalString)this.colsCodeD.Text).Scan("^") >= 0))
                {
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">An <see ref="PPJ.Runtime.WindowActionsEventArgs"/> that contains the event data.</param>
        private void colsCBCodeStr_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
