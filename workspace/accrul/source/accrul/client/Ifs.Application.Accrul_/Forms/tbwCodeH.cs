...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_H,ACCOUNTING_CODEPART_H,CODE_H_ALL", "CodeH", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeH : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeH FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeH)SalWindow.FromHandle(handle, typeof(tbwCodeH)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeH;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeH", this, sArray, hWndItems);
        ...
        private void tbwCodeH_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeH_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeH_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeH_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_H";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeH_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeH_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeH.Text).Scan("&") >= 0) || (((SalString)this.colsCodeH.Text).Scan("'") >= 0) || (((SalString)this.colsCodeH.Text).Scan("^") >= 0))
                {
