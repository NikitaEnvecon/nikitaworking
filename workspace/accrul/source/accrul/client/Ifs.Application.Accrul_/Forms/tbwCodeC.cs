...
namespace Ifs.Application.Accrul_
{
	...
	/// <summary>
	/// </summary>
    [FndWindowRegistration("CODE_C,ACCOUNTING_CODEPART_C,CODE_C_ALL", "CodeC", FndWindowRegistrationFlags.HomePage)]
    public partial class tbwCodeC : cTableWindowCodePart
    {
        ...
        /// <returns></returns>
        public new static tbwCodeC FromHandle(SalWindowHandle handle)
        {
            return ((tbwCodeC)SalWindow.FromHandle(handle, typeof(tbwCodeC)));
        }
        #endregion
        ...
        public new SalNumber DataSourcePrepareKeyTransfer(SalString sWindowName)
        {
            ...
            using (new SalContext(this))
            {
                if (sWindowName == Pal.GetActiveInstanceName("frmHistoryLog"))
                {
                    ...
                    hWndItems[2] = colsCodeC;
                    Ifs.Fnd.ApplicationForms.Var.DataTransfer.Init("CodeC", this, sArray, hWndItems);
        ...
        private void tbwCodeC_WindowActions(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            switch (e.ActionType)
            {
                case Sys.SAM_Create:
                    this.tbwCodeC_OnSAM_Create(sender, e);
                    break;

                case Ifs.Fnd.ApplicationForms.Const.PM_DataSourceQueryFieldName:
                    this.tbwCodeC_OnPM_DataSourceQueryFieldName(sender, e);
                    break;
            }
            #endregion
        }
        ...
        private void tbwCodeC_OnSAM_Create(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
            this.i_sCode = "CODE_C";
        ...
        /// </summary>
        /// <param name="message"></param>
        private void tbwCodeC_OnPM_DataSourceQueryFieldName(object sender, WindowActionsEventArgs e)
        {
            #region Actions
            e.Handled = true;
        ...
        private void colsCodeC_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sal.SendClassMessage(Ifs.Fnd.ApplicationForms.Const.PM_DataItemValidate, Sys.wParam, Sys.lParam))
            {
                ...
                if ((((SalString)this.colsCodeC.Text).Scan("&") >= 0) || (((SalString)this.colsCodeC.Text).Scan("'") >= 0) || (((SalString)this.colsCodeC.Text).Scan("^") >= 0))
                {
