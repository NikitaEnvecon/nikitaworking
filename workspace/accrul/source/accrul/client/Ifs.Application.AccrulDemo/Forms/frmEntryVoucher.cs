...
// 15-06-22    Mawelk    Bug Id 123191 Fixed in UM_CreateVouTemplate()
// 15-08-27    Mawelk    Bug Id 124231 Fixed in DataSourceFormatSqlColumnUser()
// 15-11-12    Savmlk    Bug 125580 Fixed in frmEntryVoucher_OnPM_DataSourceSave().
// 16-03-11    Nudilk    Bug 127926, Corrected.
// 16-04-05    Nudilk    Bug 128457, Corrected.
#endregion
...
namespace Ifs.Application.Accrul
{
    ...
    public partial class frmEntryVoucher : cMasterDetailTabFormWindowVou
    {
        ...
        // Bug 127926,begin.
        protected readonly SalNumber nValidRowFlags = Sys.ROW_Edited | Sys.ROW_New | Sys.ROW_UnusedFlag1;
        // Bug 127926,end.
        // Bug 128457,begin.
        protected SalString sNonManualRowExist = "FALSE";
        protected SalString sDisableInterimMenu = "FALSE";
        // Bug 128457,end.
        #endregion
                ...
                switch (nWhat)
                {
                    case Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire:
                        ...
                        if (sAuthLevel == "ApproveOnly")
                            return false;
                        // Bug 128457,removed code.
                        // Bug 128457,begin.
                        if (dfsVoucherGroup.Text != "M")
                        {
                            if (NonManualRowExist())
                                return false;
                        }
                        // Bug 128457,end.
                        return true;
        ...
        // Bug 128457,begin.
        protected virtual SalBoolean NonManualRowExist()
        {
            return (sNonManualRowExist == "TRUE") ? true : false;
        }
        // Bug 128457,end.
        /// <summary>
        /// </summary>
            ...
            using (new SalContext(this))
            {
                switch (p_nWhat)
                {
                    case Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire:
                        ...
                        // Bug Id 123191 Begin . Change the IF condition 
                        if ((cmbsVoucherStatus.Text == Sal.ListQueryTextX(cmbsVoucherStatus, 0) || cmbsVoucherStatus.Text == Sal.ListQueryTextX(cmbsVoucherStatus, 1) || Sal.ListQueryTextX(cmbsVoucherStatus, 4)) && !(Sal.SendMsg(i_hWndFrame, Ifs.Fnd.ApplicationForms.Const.PM_DataSourceSave, Ifs.Fnd.ApplicationForms.Const.METHOD_Inquire,
        ...
        // Bug 128457,begin.
        protected virtual SalBoolean DisableInterimMenu()
        {
            return (sDisableInterimMenu == "TRUE") ? true : false;
        }
        //Bug 128457,end.
        ...
        public new SalString DataSourceFormatSqlColumnUser()
        {
            ...
            using (new SalContext(this))
            {
                // Bug Id 124231 Begin,Modified method content.
                sSqlStmt = @"&AO.Authorize_Level_API.Encode(&AO.Voucher_Type_User_Group_API.Get_Authorize_Level( :i_hWndFrame.frmEntryVoucher.dfsCompany,
                										                                                        :i_hWndFrame.frmEntryVoucher.dfnAccountingYear,
                										                                                        :i_hWndFrame.frmEntryVoucher.dfsUserGroup,
                										                                                        :i_hWndFrame.frmEntryVoucher.ccsVoucherType.i_sMyValue)),
                ...
                {
                    sSqlStmt += @"&AO.Add_Investment_Info_API.Exist_Add_Investment_Info('VOUCHER',
                                                                                        COMPANY, 
                                                                                        VOUCHER_NO, 
                                                                                        VOUCHER_TYPE,
                                                                                        ACCOUNTING_YEAR, 
                ...
                // Bug 128457,begin.
                sSqlStmt += @"&AO.Voucher_Util_Pub_API.Disable_Interim_Voucher( COMPANY, 
                                                                                ACCOUNTING_YEAR,
                                                                                VOUCHER_TYPE, 
                                                                                VOUCHER_NO, 
                                                                                USER_GROUP ),";

                sSqlStmt += @"&AO.Voucher_Util_Pub_API.Non_Manual_Vou_Row_Exist( COMPANY, 
                                                                                 ACCOUNTING_YEAR,
                                                                                 VOUCHER_TYPE,
                                                                                 VOUCHER_NO)";
                // Bug 128457,end.
        ...
        public new SalString DataSourceFormatSqlIntoUser()
        {
            ...
            using (new SalContext(this))
            {
                // Bug 128457,begin,added 2 values.
                // Bug Id 124231,begin,modified parameter order.
                return ":i_hWndFrame.frmEntryVoucher.sAuthLevel, :i_hWndFrame.frmEntryVoucher.sCancellation , :i_hWndFrame.frmEntryVoucher.sSimulationVoucher, :i_hWndFrame.frmEntryVoucher.sUserGroupNew, :i_hWndFrame.frmEntryVoucher.sAddInvestment, :i_hWndFrame.frmEntryVoucher.sDisableInterimMenu, :i_hWndFrame.frmEntryVoucher.sNonManualRowExist ";
                // Bug Id 124231,end.
                // Bug 128457,end.
            }
            #endregion
        ...
        // Bug 127926,begin.
        protected virtual Boolean SetAccountingPeriodServer()
        {


            if (!(DbPLSQLBlock(cSessionManager.c_hSql, @"&AO.Voucher_Row_API.Update_Vou_Row_Acc_Period__(
                                                                                               :i_hWndFrame.frmEntryVoucher.dfsCompany                 IN,
                                                                                               :i_hWndFrame.frmEntryVoucher.dfnAccountingYear          IN,
                                                                                               :i_hWndFrame.frmEntryVoucher.dfnVoucherNo               IN,
                                                                                               :i_hWndFrame.frmEntryVoucher.ccsVoucherType.i_sMyValue  IN,
                                                                                               :i_hWndFrame.frmEntryVoucher.dfnAccountingPeriod        IN)")))
            {
                return false;
            }
            return true;
        ...
        // Bug 127926,end.
        // Bug Id 124231, removed unused method GetAddInvestmentStmt.
        #endregion
        ...
        private void frmEntryVoucher_OnPM_DataSourceSave(object sender, WindowActionsEventArgs e)
        {
            ...
            if (Sys.wParam == Ifs.Fnd.ApplicationForms.Const.METHOD_Execute)
            {
                ...
                while (Sal.TblFindNextRow(frmVoucherPosting.FromHandle(this.hWndFirstTab).tblVoucherPosting, ref this.nCurrentRow, nValidRowFlags, 0))
                {
