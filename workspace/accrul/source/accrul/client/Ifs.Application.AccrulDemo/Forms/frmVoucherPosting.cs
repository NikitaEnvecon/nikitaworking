...
// DATE        BY        NOTES
// YY-MM-DD
// 15-04-27    Chwtlk    Bug 121763, Modified SELECT query WHERE statement in CopyGLVouRow() method.
// 15-04-27    Chwtlk    Bug 121638, Modified SELECT query in CopyGLVouRow() method.
...
// 15-07-02    NUDILK    Bug 122941, Modified colnThirdCurrencyAmount_OnPM_DataItemValidate.
// 15-07-07    KANSLK    Bug 123433, Removed code introduced on PRFI-5158
// 15-08-10    THPELK    Bug 122148, Corrected.
// 16-03-11    NUDILK    Bug 127926, Corrected.
// 16-03-29    CHWTLK    Bug 128284, Modified CopyGLVouRow() and SetAnotherValue().
// 16-05-26    CHWTLK    Bug 129235, Modified tblVoucherPosting_OnSAM_FetchRowDone().
// 16-06-08    CHWTLK    Bug 129712, Corrected the issue of Parallel currency amounts getting modifed incorrectly when Currency Rate is modified.
// 16-06-08    Maaylk    Bug 129579, Fetched DecimalsInAmount when copying GL Voucher rows.
// 16-08-31    MAAYLK    Bug 131197, Modified condition to also check if colnParallelCurrRate is NOT NULL
#endregion
    ...
    public partial class frmVoucherPosting : cFormWindowFin
    {
        ...
        private void tblVoucherPosting_OnSAM_FetchRowDone(object sender, WindowActionsEventArgs e)
        {
            ...
            if (this.sAmountMethod == frmEntryVoucher.FromHandle(Ifs.Fnd.ApplicationForms.Int.GetParent(this.i_hWndFrame)).sAmountMethodGross)
            {
                ...
                if (this.tblVoucherPosting_colnGrossAmount.Number != Sys.NUMBER_Null)
                {
                    ...
                    SalNumber temp13 = this.tblVoucherPosting_colnParallelCurrGrossAmount.Number;
                    SalNumber temp14 = this.tblVoucherPosting_colnThirdCurrencyDebitAmount.Number;
                    SalNumber temp15 = this.tblVoucherPosting_colnThirdCurrencyCreditAmount.Number;
                    SalNumber temp16 = this.tblVoucherPosting_colnThirdCurrencyAmount.Number;
                    // Bug 	129235, Begin, Assigned values for temp17 and temp18 rather than 0.
                    SalNumber temp17 = this.tblVoucherPosting_colnDebetAmount.Number;
                    SalNumber temp18 = this.tblVoucherPosting_colnCreditAmount.Number;
                    // Bug 	129235, End.
                    this.ConvertGrossToNet(ref temp13, ref temp14, ref temp15, ref temp16, ref temp17, ref temp18);
                    this.tblVoucherPosting_colnParallelCurrGrossAmount.Number = temp13;
                    this.tblVoucherPosting_colnThirdCurrencyDebitAmount.Number = temp14;
                    this.tblVoucherPosting_colnThirdCurrencyCreditAmount.Number = temp15;
                    this.tblVoucherPosting_colnThirdCurrencyAmount.Number = temp16;
                }
            }
            else
            {
        ...
        public virtual SalBoolean CopyGLVouRow()
        {
            ...
            using (new SalContext(tblVoucherPosting))
            {
                ...
                // Bug  102266, Begin
                // Bug 129579, Begin. Added nDecimalsInAmount
                ...
                + "," + "PARALLEL_CURRENCY_RATE," + "PARALLEL_CONVERSION_FACTOR," + "THIRD_CURRENCY_DEBIT_AMOUNT, THIRD_CURRENCY_CREDIT_AMOUNT, NVL(THIRD_CURRENCY_DEBIT_AMOUNT,0) - NVL(THIRD_CURRENCY_CREDIT_AMOUNT,0)," + cSessionManager.c_sDbPrefix + "ACCOUNT_API.Is_Stat_Account(COMPANY,ACCOUNT), NVL(&AO.Currency_Code_API.Get_Currency_Rounding(COMPANY, CURRENCY_CODE), 0)";                     
                // Bug 102266, End
                ...
                sFormName + "tblVoucherPosting_colnParallelCurrRate," + sFormName + "tblVoucherPosting_colnParallelCurrConvFactor," + sFormName + "tblVoucherPosting_colnThirdCurrencyDebitAmount," + sFormName + "tblVoucherPosting_colnThirdCurrencyCreditAmount," + sFormName + "tblVoucherPosting_colnThirdCurrencyAmount," + sFormName + "tblVoucherPosting_colIsStatAccount," + sFormName + "tblVoucherPosting_colnDecimalsInAmount";
                // Bug 129579, End
                // Bug 121763, Begin, Modified the where statement, checked AUTO_TAX_VOU_ENTRY IS NULL AND TRANS_CODE IN ('MANUAL') 
				// Bug 121638, Begin, added AUTO_TAX_VOU_ENTRY = 'TRUE' to the where statement.
                ...
                " TRANS_CODE LIKE '%MANUAL' OR" + " AUTO_TAX_VOU_ENTRY = 'EXT' OR AUTO_TAX_VOU_ENTRY = 'TRUE') AND" + " (AUTO_TAX_VOU_ENTRY = 'FALSE' OR " + "AUTO_TAX_VOU_ENTRY = 'EXT' OR AUTO_TAX_VOU_ENTRY = 'TRUE' OR (AUTO_TAX_VOU_ENTRY IS NULL AND TRANS_CODE IN ('MANUAL')))" + 
                " ORDER BY ROW_NO " ;
                // Bug 121638, end
				// Bug 121763, end
                // Bug id 111253, end
                ...
                if (DbConnect(ref hSql))
                {
                    if (DbPrepareAndExecute(hSql, IsSelect + IsSelect2 + IsSelect3))
                    {
                        ...
                        while (DbFetchNext(hSql, ref nInd))
                        {
                            ...
                            // Bug 121638, Begin
                            if (((SalString)this.tblVoucherPosting_colsTransCode.Text).ToUpper() == "EXTERNAL")
                            {
                                bExternalVou = true;
                            }

                            if (((SalString)this.tblVoucherPosting_colsTransCode.Text).ToUpper() != "EXTERNAL" && this.tblVoucherPosting_colsOptionalCode.Text != Ifs.Fnd.ApplicationForms.Const.strNULL)
                            {
                                bTaxCodeExists = true;
                            }
                            // Bug 121638, End

                            // Bug 121638, Begin, Modified the IF condition                                
                            // Bug 102106, Begin - Modified the IF condition
                            if (bTaxCodeExists && bExternalVou)                                 
                            {
                                DataSourceClearIt(0);
                            ...
                            // Bug 102106, End
                            // Bug 121638, End
                            // Bug 121638, begin, IF condition added
                            if (this.tblVoucherPosting_colsAutoTaxVouEntry.Text != "TRUE")
                            {
                                tblVoucherPosting.i_nDataSourceState = Ifs.Fnd.ApplicationForms.Const.DATASOURCE_Changed;
        ...
        public virtual SalBoolean SetAccountingPeriod(SalNumber nPeriod)
        {
            ...
            SalNumber nRow = Sys.TBL_MinRow;
            // Bug 127926,begin,modified code.                    
        ...
        public virtual SalNumber SetAnotherValue(SalNumber nMyValue, SalWindowHandle hWndRival, SalString sStatus, ref SalNumber nCalculatedTaxValue)
        {
                    ...
                    if ((tblVoucherPosting_colnCurrencyAmount.Number != 0) && (tblVoucherPosting_colnCurrencyAmount.Number != Sys.NUMBER_Null))
                    {
                        ...
                        {   
                            ...
                            if ((tblVoucherPosting_colsCurrencyCode.Text != this.sCurrCode) && (this.IsEmu == "FALSE"))
                            {
                                ...
                                // Bug  129712, Begin.
                                if (this.i_sFinParallelBaseType == "ACCOUNTING_CURRENCY" && (tblVoucherPosting_colsCurrencyCode.Text == this.i_sFinThirdCurrencyCode && this.i_sFinThirdCurrencyCode != Ifs.Fnd.ApplicationForms.Const.strNULL))
                                {
                                    tblVoucherPosting_colnParallelCurrRate.Text = tblVoucherPosting_colnCurrencyRate.Text;
                                }
                                // Bug  129712, End.
            ...
            // Bug 128284, Begin, Checked bCopyFromGL, SetThirdCurrencyAmounts() is not called if Copy GL voucher Rows is used to create the voucher.
            // Third currency calculation
            if (bCalculateParallelCurrTax && !bCopyFromGL)
            {
                SetThirdCurrencyAmounts();
            }
            // Bug 128284, End.
        ...
        public virtual SalNumber CalcualteParallelCurrTaxValue(SalString sBasedOn)
        {
            ...
            if (frmEntryVoucher.FromHandle(Ifs.Fnd.ApplicationForms.Int.GetParent(i_hWndFrame)).sParallelCurrency != Ifs.Fnd.ApplicationForms.Const.strNULL)
            {
                ...
                if (sBasedOn == Sys.STRING_Null || sBasedOn == "RATE")
                {
                    ...
                    {
                        // Bug 131197, Begin. Added a NULL check
                        if (tblVoucherPosting_colnParallelCurrRate.Number != 0 && this.tblVoucherPosting_colnParallelCurrRate.Number != SalNumber.Null)
                        {
        ...
        private void tblVoucherPosting_colnParallelCurrRateType_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            #region Actions
            e.Handled = true;
            // Bug  129712, Begin, Added a condition to check whether Parallel Base type is ACCOUNTING_CURRENCY and used Currency Code for that line is the Third Currency.
            if (!(this.i_sFinParallelBaseType == "ACCOUNTING_CURRENCY" && (tblVoucherPosting_colsCurrencyCode.Text == this.i_sFinThirdCurrencyCode && this.i_sFinThirdCurrencyCode != Ifs.Fnd.ApplicationForms.Const.strNULL)))
            {
        ...
        private void tblVoucherPosting_colnThirdCurrencyAmount_OnPM_DataItemValidate(object sender, WindowActionsEventArgs e)
        {
            ...
            {
                // Bug 122941,begin,added if condition and moved existing code to else.
                if (this.tblVoucherPosting_colnCurrencyCreditAmount.Number != Sys.NUMBER_Null && this.tblVoucherPosting_colnCurrencyDebetAmount.Number == Sys.NUMBER_Null)
                {
                    this.tblVoucherPosting_colnThirdCurrencyCreditAmount.Number = this.tblVoucherPosting_colnThirdCurrencyAmount.Number;
                    this.tblVoucherPosting_colnThirdCurrencyCreditAmount.EditDataItemSetEdited();
                    this.tblVoucherPosting_colnThirdCurrencyDebitAmount.Number = Sys.NUMBER_Null;
                    this.tblVoucherPosting_colnThirdCurrencyDebitAmount.EditDataItemSetEdited();
                }
                else
                {
